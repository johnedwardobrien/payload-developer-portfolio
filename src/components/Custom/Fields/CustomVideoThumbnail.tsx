...
export const CustomVideoThumbnail: RelationshipFieldClientComponent = (
	props,
) => {
	const { path } = props;
	...
	useEffect(() => {
		if (fields?.videoThumbnailFilename?.value && !thumbnailUrl) {
			const defaultThumbnailUrl = `${process.env.NEXT_PUBLIC_VERCEL_BLOB_URL || ''}${fields.videoThumbnailFilename?.value}`;
	...
	async function uploadStoredThumbnail() {
		...
		try {
			...
			if (uploadedThumbnail) {
				...
				dispatchFields({ type: 'UPDATE', path: 'videoThumbnailFilename', value: uploadedThumbnail.filename });
	...
	useEffect(() => {
		async function generatePreview() {
			...
			if (file?.value && typeof file.value === 'object' && 'type' in file.value && 
				typeof file.value.type === 'string' && file.value.type.includes('video')) {
	...
	return <div>
		{thumbnailUrl && <img src={thumbnailUrl} style={{ maxWidth: '500px', height: 'auto' }} alt="Video thumbnail" />}
