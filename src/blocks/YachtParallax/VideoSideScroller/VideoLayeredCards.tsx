...
export const VideoLayeredCards: React.FC<Props> = ({
  ...
  const containerWaypoints = Array.from({ length: cards.length }, (_, i) => i / cards.length)
    ...
    <div className={`video-layered-cards-wrapper ${isDesktop ? 'desktop-layout' : ''}`}>
      {isDesktop && (
        <div className="video-layered-cards-left-column">
          <div className="content">
            ...
            <div className="inner">
              <AnimatePresence mode="wait">
                ...
                >
                  {currentCard?.title && (
                    <h3 className="video-layered-cards-title">{currentCard.title}</h3>
                  )}
          ...
          <div className="title-card-cont" ref={titleCardContRef}>
            {titleCards.map((card) => {
              return (
                <span
                  key={card.cardId || card.index}
                  ref={(el) => {
                    titleCardRefs.current[card.index] = el
                  }}
              ...
              return (
                <motion.div
                  ...
                  style={{
                    zIndex,
                    opacity:
                      isActive || isNextCard || (isPreviousCard && isPreviousSlidingIn) ? 1 : 0,
                    x: isDesktop ? 0 : isActive ? x : isPreviousCard ? prevX : 0,
                    pointerEvents: isActive && !isDesktop ? 'auto' : 'none',
                    cursor:
                      isActive && !isDesktop && (canSwipeLeft || canSwipeRight)
                        ? 'grab'
                        : 'default',
                  }}
                  drag={
                    isDesktop ? false : isActive && (canSwipeLeft || canSwipeRight) ? 'x' : false
                  }
                  dragConstraints={
